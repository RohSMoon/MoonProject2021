...
public class Config_Status_Change_Panel extends JPanel {
	...
	DefaultTableModel config_Status_dtm                       		 	; // 유저 정보 보여줄 데이타 테이믈 모델
	JTable	 		  showStatustable	                                ; // 유저 정보 보여줄 테이블
	JLabel			  showInfo_jbl								        ; // 패널에 붙일 타이틀 라벨 
	String 				cols[]		= {"번호","제목","인원"} 	 			; // 테이블 헤더 컬럼
	String 		    	data[][] 	= new String[0][3]	  				; // 테이블  테이블 크기 초기화
	...
	public void Initialize() {	
		...
		this.add(showStatustable = new JTable()); // 테이블 생성 및 패널이 붙이기 
		...
		config_Status_dtm = new DefaultTableModel(data,cols)
		); 	
